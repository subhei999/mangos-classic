...
pub(in crate::world) struct GameEventState {
    pub(in crate::world) active_events: HashSet<u16>,
}

impl GameEventState {
    pub(in crate::world) fn from_schedules_at(
        schedules: &[wow_db::GameEventScheduleQuery],
        now_unix: i64,
    ) -> Self {
    ...
    pub(in crate::world) fn is_active(&self, event_id: u16) -> bool {
        self.active_events.contains(&event_id)
    }

    pub(in crate::world) fn spawn_is_active(&self, game_event: Option<i16>) -> bool {
        match game_event {
    ...
    pub(in crate::world) fn active_count(&self) -> usize {
        self.active_events.len()
    }
}

pub(in crate::world) fn game_event_is_active_at(
...
pub(in crate::world) fn yearly_game_event_is_active_at(
    schedule: &wow_db::GameEventScheduleQuery,
    now_unix: i64,
) -> bool {
    let Some(start) = schedule.start_time_unix else {
    ...
    let (_, start_month, start_day, start_hour, start_minute, start_second) = unix_to_ymdhms(start);
    ...
    for year in [now_year - 1, now_year, now_year + 1] {
        ...
        let Some(mut end_for_year) = ymdhms_to_unix(
...
pub(in crate::world) fn game_event_interval_is_active_at(
    schedule: &wow_db::GameEventScheduleQuery,
    start: i64,
    end: i64,
    now_unix: i64,
...
pub(in crate::world) fn unix_to_ymdhms(unix: i64) -> (i32, u32, u32, u32, u32, u32) {
...
pub(in crate::world) fn ymdhms_to_unix(
    year: i32,
    month: u32,
    day: u32,
    hour: u32,
...
pub(in crate::world) fn days_from_civil(year: i32, month: u32, day: u32) -> i64 {
...
pub(in crate::world) fn civil_from_days(days: i64) -> (i32, u32, u32) {
