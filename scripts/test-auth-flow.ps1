...
try {
    ...
    if ($server.HasExited) {
        throw "authserver exited during startup with code $($server.ExitCode)"
    }

    Invoke-Checked cargo @("run", "-p", "auth-flow-test")
}
finally {
    if (-not $KeepRunning -and -not $server.HasExited) {
