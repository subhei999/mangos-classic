...
async fn unauthorized_creature_loot_request_sends_cmangos_loot_error() {
    ...
    let response = packets
        .iter()
        .find(|packet| packet.opcode == SMSG_LOOT_RESPONSE)
...
fn map_runtime_db_creature_spell_completion_rechecks_range_and_los() {
    ...
    let opcodes = interrupted
        ...
        .collect::<Vec<_>>();
    assert!(opcodes.contains(&SMSG_SPELL_FAILURE));
    assert!(opcodes.contains(&SMSG_SPELL_FAILED_OTHER));
...
fn map_runtime_db_creature_spell_cast_drops_if_victim_dies_before_go() {
    ...
    assert!(interrupted
        .observer_packets
        .iter()
        .any(|(_, packet)| packet.opcode == SMSG_SPELL_FAILURE));
    assert!(interrupted
        .observer_packets
        .iter()
        .any(|(_, packet)| packet.opcode == SMSG_SPELL_FAILED_OTHER));
    assert!(map
        .active_db_creature_spell_cast_due_at(creature_guid)
        .is_none());
}
...
fn map_runtime_weapon_spell_avoid_sends_spell_miss_log_without_damage_log() {
    ...
    assert!(event
        .observer_packets
        .iter()
        .any(|(_, packet)| packet.opcode == SMSG_SPELLLOGMISS));
...
fn db_creature_movement_scripts_execute_emote_and_morph_from_db_data() {
    ...
    assert!(packets.iter().all(|(session, packet)| {
        *session == SessionId(1) && packet.opcode == SMSG_UPDATE_OBJECT
    }));
...
fn db_creature_movement_scripts_send_monster_talk_from_broadcast_text() {
    ...
    assert_eq!(packet.opcode, SMSG_MESSAGECHAT);
...
fn creature_dot_death_clears_auras_before_respawn() {
    ...
    assert!(packets
        .iter()
        .all(|(_, packet)| packet.opcode != SMSG_PERIODICAURALOG));
}
...
fn map_owned_consumable_regen_aura_ticks_health_and_mana() {
    ...
    assert!(packets
        .iter()
        .any(|(_, packet)| packet.opcode == SMSG_SPELLENERGIZELOG));
}
...
fn map_runtime_magma_environmental_timer_applies_lava_damage_without_client_timer() {
    ...
    assert!(start_packets
        .iter()
        .all(|(_, packet)| packet.opcode != SMSG_START_MIRROR_TIMER));
...
async fn sinister_strike_cast_uses_energy_and_spell_damage_log_result() {
    ...
    let spell_damage = packets
        .iter()
        .find(|packet| packet.opcode == SMSG_SPELLNONMELEEDAMAGELOG)
...
async fn backstab_cast_requires_caster_behind_target_from_spell_metadata() {
    ...
    assert!(packets.iter().any(|packet| {
        packet.opcode == SMSG_CAST_RESULT && packet.body[5] == SPELL_FAILED_NOT_BEHIND
    }));
    assert!(!packets
        .iter()
        .any(|packet| packet.opcode == SMSG_ATTACKERSTATEUPDATE));
...
async fn eviscerate_requires_matching_combo_points_before_spending_energy() {
    ...
    let packets = std::iter::from_fn(|| rx.try_recv().ok()).collect::<Vec<_>>();
    assert!(packets.iter().any(|packet| {
        packet.opcode == SMSG_CAST_RESULT && packet.body[5] == SPELL_FAILED_NO_COMBO_POINTS
    }));
}
...
async fn player_heal_spell_cast_restores_self_health_through_map_runtime() {
    ...
    assert!(opcodes.contains(&SMSG_SPELL_GO));
    assert!(opcodes.contains(&SMSG_SPELLHEALLOG));
    assert!(opcodes.contains(&SMSG_UPDATE_OBJECT));
    assert!(!opcodes.contains(&SMSG_SPELLNONMELEEDAMAGELOG));
    let heal_log = packets
        .iter()
        .find(|packet| packet.opcode == SMSG_SPELLHEALLOG)
...
async fn cast_time_spell_sends_start_before_delayed_go_and_effects() {
    ...
    let opcodes = packets
        ...
        .map(|packet| packet.opcode)
        .collect::<Vec<_>>();
    assert_eq!(opcodes[0], SMSG_SPELL_START);
    ...
    assert!(opcodes.contains(&SMSG_SPELLNONMELEEDAMAGELOG));
    assert!(
        !opcodes.contains(&SMSG_ATTACKERSTATEUPDATE),
...
async fn cast_time_spell_rechecks_facing_before_completion_go() {
    ...
    let packets = std::iter::from_fn(|| rx.try_recv().ok()).collect::<Vec<_>>();
    assert!(packets.iter().any(|packet| {
        packet.opcode == SMSG_CAST_RESULT && packet.body[5] == SPELL_FAILED_UNIT_NOT_INFRONT
    }));
    assert!(!packets.iter().any(|packet| packet.opcode == SMSG_SPELL_GO));
    assert!(!packets
        .iter()
...
fn raptor_strike_spell_packets_match_success_shapes() {
    let caster = ObjectGuid::new(HighGuid::Player, 0, 7);
    let target = ObjectGuid::new(HighGuid::Unit, 6, 45);
    let spell_profile = player_spell_cast_profile(&raptor_strike_spell_template()).unwrap();
