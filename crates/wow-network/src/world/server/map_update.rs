...
pub(in crate::world) async fn run_map_runtime_update_loop(runtime_state: WorldRuntimeState) {
    ...
    loop {
        ...
        match runtime_state
            .maps
            .advance_all_dynamic_objects(now, current_unix_epoch_secs())
            .await
        {
            Ok(packets) => {
                crate::observability::record_map_phase_duration(
            ...
            Err(error) => {
                ...
                crate::observability::record_map_tick_error();
                warn!("Map runtime dynamic-object tick failed: {error}");
            }
        }
        let phase_started_at = Instant::now();
        match runtime_state
            .maps
            .advance_all_player_channels(now, current_unix_epoch_secs())
            .await
        {
            Ok(packets) => {
                crate::observability::record_map_phase_duration(
            ...
            Err(error) => {
                ...
                crate::observability::record_map_tick_error();
                warn!("Map runtime player-channel tick failed: {error}");
            }
        }
        let phase_started_at = Instant::now();
        match runtime_state
        ...
        match runtime_state
            .maps
            .advance_all_player_death_presentations(now)
            .await
        {
            Ok(packets) => {
                crate::observability::record_map_phase_duration(
