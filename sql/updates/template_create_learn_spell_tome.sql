-- Template: create a "learn spell" tome item by cloning Tome of Arcane Brilliance (18600)
-- and overriding only the few fields needed for the `item_hardcore_ability` script.
--
-- Notes:
-- - This template is useful for *one-off* items.
-- - For bulk (hundreds/thousands), use: `sql/updates/generate_learn_spell_tomes.py`
--
-- Required:
-- - ScriptDevAI compiled/enabled
-- - ScriptName `item_hardcore_ability` registered and loaded
--
-- Variables:
SET @TEMPLATE_ENTRY := 18600; -- Tome of Arcane Brilliance
SET @NEW_ENTRY      := 90002;
SET @SPELL_TO_LEARN := 143;
SET @ITEM_NAME      := 'Tome of Fireball (Rank 2)';
