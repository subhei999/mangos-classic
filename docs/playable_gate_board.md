...
6. In progress: move DB creature live state into the shared runtime so all
   ...
   through `MapRuntime`, new idle motion splines broadcast through the shared
   map to nearby observers, and chase start/stop, evade prep, and assistance
   calls are map-owned. Creature-origin damage packet execution and lifecycle
   event broadcast authority are in; remaining G12 work is broader torture
   coverage and runtime cleanup rather than session-local lifecycle ownership.
